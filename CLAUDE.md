...
- `postgres` driver (`needs_schema: true`) is special-cased: instead of an isolated temp process, it runs against one courseforge-managed Postgres cluster, isolated per run via `CREATE SCHEMA`/`DROP SCHEMA` rather than a new database — the cluster's database count never grows. Managed by `PostgresManager`, started/stopped in `internal/di/di.go`. Two platform implementations: `postgres.go` (Unix, connects over a private Unix socket dir) and `postgres_windows.go` (Windows, connects over loopback TCP on a port picked at startup, since Postgres on Windows has no socket-file support).
